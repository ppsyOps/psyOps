...
def datestr_for_filename(datetime_val, precision='seconds'):
    #from datetime import datetime
    ret = str(datetime_val).replace(' ','_').replace(':','').replace('.','') 
    precision=precision[:2]
    if precision[0] != 'm': precision=precision[0]
    trim_dict = {'y':4, 'mo':7, 'mm':7, 'd':10, \
    'h':13,  'mi':15, 's':17, 'ms':23}
    i = int( trim_dict[precision])
    print i
    if i>0 and precision != None:
        ret=ret[:i]
    return ret

def datestr(datetime_val, fmt='USA', precision='seconds'):
    #from datetime import datetime
    dt = datetime_val
    if fmt[:2] == 'US': # USA
        dts = ('0'+str(dt.month))[:2]
        dts += '/' + ('0'+str(dt.day))[:2]
        dts += '/' + str(dt.year)
        dts += ' ' + ('0'+str(dt.hour))[:2]
        dts += ':' + ('0'+str(dt.minute))[:2]
        dts += ':' + ('0'+str(dt.second))[:2]
        dts += '.' + str(dt.microsecond)
    elif fmt[:2] == 'IS': # ISO 8601: 2016-04-25T14:48:57
        dts = str(dt).replace(' ','T') 
    elif fmt[:2] == 'FI': # file: 2016-04-25_144857
        dts = str(dt).replace(' ','_').replace(':','').replace('.','') 
    precision=precision[:2]
    if precision[0] != 'm': 
